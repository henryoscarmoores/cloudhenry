...
$codes = if ($OnlyOrigins) { @($OnlyOrigins | ForEach-Object { $_ -split "," } | Where-Object { $_ }) } else { @("MAN","BHX","LBA","STN","LTN","BRS","NCL","GLA","EDI","LGW","LPL","BFS","BOH","CWL","EMA","DUB","EXT","LHR","SEN","LCY","PIK","BHD","ORK","SNN","SOU","ABZ","NWI","NQY","NOC","MME","INV","HUY","JER","GCI","IOM","KIR","LDY","DND") }
foreach ($o in $codes) {
