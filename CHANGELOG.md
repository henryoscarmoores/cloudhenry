...
- Security pass against the "vibe coded app" checklist (6 Sep evening): no secrets in the public repo or its history, none in browser scripts, tokens travel in headers only. One-tap sign-in links (/go) are now signed over destination and expiry with the Ghost secret, expire after three weeks, and the member's uuid and Ghost %%{key}%% are checked with Ghost before a sign-in is issued (5562b43); the 14 Monday drafts were rebuilt with the new links. Planner errors no longer echo Anthropic's message to the browser.
